...
Добавление времени прибытияавтобуса по его номеру и времени в формате «hh:mm».
...
Метод getIntHours(): Так как при работе со временем удобней использовать не символы, а числовые значения, то я решил создать отдельный метод, который будет возвращать количество часов в типе данных int.
```java
...
Метод getIntMinutes(): Так как при работе со временем удобней использовать не символы, а числовые значения, то я решил создать отдельный метод, который будет возвращать количество минут в типе данных int.
```java
...
Второй блок реализация и тесты на 3 и 4 задание
При этом часть кода которая была в первом блоке тоже должна присутствовать (я решил ее не добовлять чтобы сосредоточить внимание на новом блоке)
...
Удалили автобус 210 и время 17:01 у автобуса 440

5. Тест на 3 и 4 задания:
    ...
    - **Output**:
        ```
             ...
             // 2 Блок
             430 : 09:00, 10:00, 12:00, 15:30, 18:10, 23:10
             440 : 09:25, 10:30, 11:40, 13:00, 19:10
             550 : 03:40, 21:25, 23:30

        ```

Третий блок реализация и тесты на 5 и 6 задание
При этом часть кода которая была в предыдущих блоках тоже должна присутствовать (я решил ее не добовлять чтобы сосредоточить внимание на новом блоке)
...
Создали два периодических автобуса 340р и 240р. 340р - делает четыре остановки начиная с 08:30 каждые два с половиной часа. 240р - делает остановки до 22:00 начиная с 10:15 каждые 3 часа.

5. Тест на 5 и 6 задания:
    ...
    - **Output**:
        ```
             ...
             440 : 09:25, 10:30, 11:40, 13:00, 19:10
             550 : 03:40, 21:25, 23:30
             340p : 08:30, 11:00, 13:30, 16:00
             240p : 10:15, 13:15, 16:15, 19:15
        ```

Четвертый блок реализация и тесты на 7, 8, 9 и 10 задание
При этом часть кода которая была в предыдущих блоках тоже должна присутствовать (я решил ее не добовлять чтобы сосредоточить внимание на новом блоке)
...
public class Test {
    public static void main(String[] args){
        ...
        // Создаем массив желаемых автобусов
        String[] list = new String[3];
        list[0] = "440";
        list[1] = "340p";
        list[2] = "550";
        // Какой автобус приедет первый из массива нужных если человек прийдет на остановку в 15:00 (Задание 10)
        System.out.println(raspis.firstBusForPerson("15:00", list));
...
Возвращаем номер автобуса который приедет первый из массив нужных номеров (массив создан выше он состоит из 440, 340р и 550 автобусов) если человек прийдет на остановку в 15:00 (Задание 10)

5. Тест на 7, 8, 9  и 10 задания:
